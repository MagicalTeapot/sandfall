...
void update_sand(pixel_api&& api, const world_settings& world, double dt)
{
    auto& vel = api.get({0, 0}).velocity;
    vel += world.gravity * (float)dt;
...
void update_water(pixel_api&& api, const world_settings& world, double dt)
{
    auto& vel = api.get({0, 0}).velocity;
    vel += world.gravity * (float)dt;
